...
import { Search, Plus, Eye, CheckCircle, PlayCircle, RefreshCw, Trash2, X, ChevronLeft, ChevronRight } from 'lucide-react';
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map(batch => (
                  <tr key={batch.id} className="border-b border-[#e2e8f0] hover:bg-[rgba(13,148,136,0.03)] transition-colors">
                    <td className="px-4 py-3 text-sm text-[#1e293b] font-mono">{batch.id}</td>
                    <td className="px-4 py-3 text-sm text-[#1e293b] font-medium">{batch.name}</td>
                    ...
                    <td className="px-4 py-3 text-sm text-[#1e293b]">${batch.totalAmount.toLocaleString()}</td>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  ...
                  <p className="text-sm font-mono text-[#1e293b]">{viewingBatch.id}</p>
                </div>
                <div className="bg-[#f8fafc] rounded-lg p-3">
                ...
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  ...
                  <p className="text-sm text-[#1e293b]">{viewingBatch.totalBeneficiaries.toLocaleString()}</p>
                </div>
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  <p className="text-xs text-[#475569] mb-1">Total Amount</p>
                  <p className="text-sm font-medium text-[#0d9488]">${viewingBatch.totalAmount.toLocaleString()}</p>
                </div>
              </div>
              {viewingBatch.validatedAt && (
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  <p className="text-xs text-[#475569] mb-1">Validated At</p>
                  <p className="text-sm text-[#1e293b]">{new Date(viewingBatch.validatedAt).toLocaleString()}</p>
                </div>
              )}
              {viewingBatch.executedAt && (
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  <p className="text-xs text-[#475569] mb-1">Executed At</p>
                  <p className="text-sm text-[#1e293b]">{new Date(viewingBatch.executedAt).toLocaleString()}</p>
                </div>
              )}
            </div>
