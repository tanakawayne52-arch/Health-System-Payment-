...
export default function BeneficiariesPage() {
  ...
  const handleSave = (formData: Partial<Beneficiary>) => {
    ...
    if (editingBen) {
      ...
      logs.unshift({
        id: generateId('a'), userId: user?.id || '', userName: user?.fullName || '', userRole: user?.role || '',
      ...
      const newBen: Beneficiary = {
        id: generateId('b'), fullName: formData.fullName || '', nationalId: formData.nationalId || '',
          ...
          <table className="w-full">
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                paginated.map(ben => (
                  <tr key={ben.id} className="border-b border-[#e2e8f0] hover:bg-[rgba(13,148,136,0.03)] transition-colors group">
                    <td className="px-4 py-3">
                      <div className="relative group/tooltip">
                        <span className="text-sm text-[#1e293b] font-medium cursor-pointer hover:text-[#0d9488]">{ben.fullName}</span>
                    ...
                    <td className="px-4 py-3">
                      <div className="relative group/tooltip">
                        <div className="flex items-center gap-1">
                          <span className="text-sm text-[#475569] font-mono cursor-pointer hover:text-[#0d9488]">{ben.nationalId}</span>
                          <button onClick={() => { navigator.clipboard.writeText(ben.nationalId); addToast('National ID copied to clipboard', 'success'); }} className="text-[#94a3b8] hover:text-[#0d9488] opacity-0 group-hover:opacity-100 transition-opacity">
                            <Copy className="w-3 h-3" />
                          </button>
                        </div>
                        <div className="absolute left-0 bottom-full mb-2 hidden group-hover/tooltip:block bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap z-50">
                          Click to copy National ID
                          <div className="absolute left-2 top-full border-4 border-transparent border-t-gray-900" />
                        </div>
                      </div>
                    </td>
