...
import { FileBarChart, Users, Wallet, AlertTriangle, CreditCard, ExternalLink, ArrowRight } from 'lucide-react';
      ...
      {activeTab === 'summary' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div className="bg-white rounded-lg shadow-[0_1px_3px_rgba(0,0,0,0.08)] p-5 hover:shadow-[0_4px_12px_rgba(0,0,0,0.12)] transition-shadow cursor-pointer group" onClick={() => navigate('/payment-batches')}>
            ...
            <table className="w-full">
              ...
              <tbody>
                {provinceData.map(row => (
                  <tr key={row.province} className="border-b border-[#e2e8f0] hover:bg-[rgba(13,148,136,0.03)] transition-colors group">
                    <td className="px-4 py-3">
                      <div className="relative group/tooltip">
                        <button onClick={() => navigate('/payment-lists')} className="text-sm text-[#1e293b] font-medium hover:text-[#0d9488] flex items-center gap-1">
                          {row.province}
                        ...
                        <div className="absolute left-0 bottom-full mb-2 hidden group-hover/tooltip:block bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap z-50">
                          View payment lists for {row.province}
