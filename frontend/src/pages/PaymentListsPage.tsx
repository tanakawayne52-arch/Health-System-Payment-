...
import { Search, Plus, Eye, Pencil, Send, Trash2, X, ChevronLeft, ChevronRight, ExternalLink, Info, Calendar, Users, DollarSign } from 'lucide-react';
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                paginated.map(list => {
                  ...
                  return (
                    <tr key={list.id} className="border-b border-[#e2e8f0] hover:bg-[rgba(13,148,136,0.03)] transition-colors group">
                      <td className="px-4 py-3">
                        <div className="relative group/tooltip">
                          <span className="text-sm text-[#1e293b] font-medium cursor-pointer hover:text-[#0d9488] underline decoration-dotted">{list.name}</span>
                      ...
                      <td className="px-4 py-3">
                        <div className="relative group/tooltip">
                          <button onClick={() => navigate('/payment-cycles')} className="text-sm text-[#475569] hover:text-[#0d9488] flex items-center gap-1">
                            <Calendar className="w-3 h-3" />
                            {cycle?.name || list.cycleId}
                            <ExternalLink className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                          </button>
                          <div className="absolute left-0 bottom-full mb-2 hidden group-hover/tooltip:block bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap z-50">
                            View payment cycle details
                            <div className="absolute left-2 top-full border-4 border-transparent border-t-gray-900" />
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="relative group/tooltip">
                          <div className="flex items-center gap-1 text-sm text-[#1e293b]">
                            ...
                            {list.beneficiaryCount.toLocaleString()}
                          </div>
                          <div className="absolute left-0 bottom-full mb-2 hidden group-hover/tooltip:block bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap z-50">
                            Total beneficiaries in this list
                            <div className="absolute left-2 top-full border-4 border-transparent border-t-gray-900" />
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="relative group/tooltip">
                          <div className="flex items-center gap-1 text-sm text-[#1e293b] font-medium">
                            ...
                            ${list.totalAmount.toLocaleString()}
                          </div>
                          <div className="absolute left-0 bottom-full mb-2 hidden group-hover/tooltip:block bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap z-50">
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-[600px] w-full mx-4 p-6">
            ...
            <div className="space-y-3">
              <div className="bg-[#f8fafc] rounded-lg p-4 border-l-4 border-[#0d9488]">
                <p className="text-xs text-[#475569] mb-1">List ID</p>
                <p className="text-sm font-mono text-[#1e293b] flex items-center gap-2">
                  {viewingList.id}
                  <button onClick={() => { navigator.clipboard.writeText(viewingList.id); addToast('ID copied to clipboard', 'success'); }} className="text-[#94a3b8] hover:text-[#0d9488]">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  ...
                  <button onClick={() => { setViewingList(null); navigate('/payment-cycles'); }} className="text-sm text-[#0d9488] hover:underline flex items-center gap-1">
                    {cycles.find(c => c.id === viewingList.cycleId)?.name || viewingList.cycleId}
                ...
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  ...
                  <p className="text-sm text-[#1e293b] flex items-center gap-1">
                    <Users className="w-3 h-3 text-[#0d9488]" />
                    {viewingList.beneficiaryCount.toLocaleString()}
                  </p>
                </div>
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  ...
                  <p className="text-sm font-medium text-[#0d9488] flex items-center gap-1">
                    ...
                    ${viewingList.totalAmount.toLocaleString()}
                  </p>
                </div>
              </div>
              {viewingList.submittedAt && (
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  ...
                  <p className="text-sm text-[#1e293b] flex items-center gap-1">
                    ...
                    {new Date(viewingList.submittedAt).toLocaleString()}
                  </p>
                </div>
              )}
              {viewingList.reviewedBy && (
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  <p className="text-xs text-[#475569] mb-1">Reviewed By</p>
                  <p className="text-sm text-[#1e293b]">{viewingList.reviewedBy}</p>
                </div>
              )}
              {viewingList.certificationNotes && (
                <div className="bg-[#f8fafc] rounded-lg p-3">
                  <p className="text-xs text-[#475569] mb-1">Certification Notes</p>
                  <p className="text-sm text-[#1e293b]">{viewingList.certificationNotes}</p>
                </div>
              )}
            </div>
